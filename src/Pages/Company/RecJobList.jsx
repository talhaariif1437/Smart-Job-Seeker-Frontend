...
const RecommendedJob = ({ job, JobseekerOptions }) => {
  ...
  return (
    // Adjusted class names for consistent card sizing and layout
      <div className="job-card border w-[300px] shadow-md drop-shadow-sm shadow-slate-400 relative flex-row justify-between max-h-min p-3 rounded-xl  mb-5 text-black">
        {/* {viewActionContainer && (
          <div
        ...
        <div>
          <div className="flex items-start justify-between  text-black ">
            <div className="flex items-center ">
              {job?.logo && (
                <img className="h-20 w-20 rounded-full text-black " src={job?.logo} alt="Company Logo" />
              )}
              <div className="flex flex-col text-left pl-4 ">
                {/* <p className={`text-[18px] font-medium m-0  ${jobCard?.singleTruncate}`}>
                  {job?.companyName}
                </p> */}
                <p className={`text-[14px] font-light text-black m-0 title-length`}>
...
const RecJobList = () => {
  ...
  useEffect(()=>{
    axios
    .get("https://smartjobseeker-fe218b533e4f.herokuapp.com/api/job/jobRecommend"
      ,{
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      console.log(response.data)
        setJobs(response?.data?.matchingJobs)
  ...
  borderRadius: '8px'
      ...
      {jobs.map((job) => (
        <RecommendedJob key={job._id} job={job} JobseekerOptions={<div>Options here</div>} />
      ))}
    </div>
    </>
